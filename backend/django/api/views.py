...
def predict(request):
    if request.method == 'POST':
        ...
        if not all([flair_file, t1ce_file, t2_file, mask_file]):
            return HttpResponse("Please upload all 4 files: FLAIR, T1CE, T2, and Mask.")
...
def api_mri_process(request):
    ...
    try:
        ...
        # Load model (simple relative path)
        model_path = os.path.join(settings.BASE_DIR, 'model', 'model-V2-diceLoss_focal.keras')
        if not os.path.exists(model_path):
            return JsonResponse({
                'success': False,
                'error': f'AI model not found at: {model_path}. Please contact administrator.'
            }, status=500)
